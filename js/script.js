...
function displayValue() {
  ...
  const html = matchArray.map(place => {
    ...
    const cityName = place.city.replace(reg, `<span class = 'highlighted'>${this.value}</span>`);
    const stateName = place.state.replace(reg, `<span class = 'highlighted'>${this.value}</span>`);
    return `
